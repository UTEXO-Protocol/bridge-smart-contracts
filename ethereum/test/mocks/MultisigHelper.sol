...
library MultisigHelper {
    ...
    bytes32 internal constant PROPOSE_DISABLE_LZ_ADAPTER_TYPEHASH =
    ...
    function domainSeparator(address verifyingContract, uint256 chainId) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(DOMAIN_TYPEHASH, keccak256("MultisigProxy"), keccak256("1"), chainId, verifyingContract)
            );
    }
    ...
    function digestTeeFundsOut(bytes32 domainSep, IBridge.FundsOutParams memory p, uint256 nonce, uint256 deadline)
        internal
        pure
        returns (bytes32)
    {
    ...
    function digestEmergencyUnpause(bytes32 domainSep, uint256 nonce, uint256 deadline)
        internal
        pure
        returns (bytes32)
    {
    ...
    function digestProposeUpdateBridge(bytes32 domainSep, address newBridge, uint256 nonce, uint256 deadline)
        internal
        pure
        returns (bytes32)
    {
    ...
    function digestProposeUpdateLZAdapter(bytes32 domainSep, address newLZAdapter, uint256 nonce, uint256 deadline)
        internal
        pure
        returns (bytes32)
    {
        return toTypedDataHash(
            domainSep, keccak256(abi.encode(PROPOSE_UPDATE_LZ_ADAPTER_TYPEHASH, newLZAdapter, nonce, deadline))
        );
    }
