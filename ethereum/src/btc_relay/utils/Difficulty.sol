...
import {
    TARGET_TIMESPAN,
    TARGET_TIMESPAN_DIV_4,
    TARGET_TIMESPAN_MUL_4,
    ROUNDED_MAX_TARGET,
    ROUNDED_MAX_TARGET_NBITS
} from "../Constants.sol";
...
library Difficulty {
    ...
    function computeNewTarget(uint32 prevTimestamp, uint32 startTimestamp, uint32 prevNbitsLE, bool clampTarget)
        internal
        pure
        returns (uint256 newTarget, uint32 newNbitsLE)
    {
        uint256 timespan = uint256(prevTimestamp) - uint256(startTimestamp);
        //Difficulty increase/decrease multiples are clamped between 0.25 (-75%) and 4 (+300%)
        if (timespan < TARGET_TIMESPAN_DIV_4) timespan = TARGET_TIMESPAN_DIV_4;
        ...
        assembly {
            ...
            let nWord :=
                or(
                    or(and(shl(16, prevNbitsLE), 0x7f000000), and(prevNbitsLE, 0xff0000)),
            ...
            newNbitsLE := or(
                or(and(shl(16, newNWord), 0xff000000), and(newNWord, 0xff0000)),
                or(and(shr(16, newNWord), 0xff00), nSize)
            )
        }
    ...
    function getChainWork(uint256 target) internal pure returns (uint256 chainwork) {
        assembly {
